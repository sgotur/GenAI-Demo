...
with Diagram("GenAI_document_processor_architecture", show=False):
    users = Users("Demo Users")
    demo_app = Python("Streamlit Python App \n select industry and document")
    context_extraction = Textract("Detect document text")

    with Cluster("Process content"):
    ...
    with Cluster("Summarization"):
        model_process1 = SagemakerModel("Generative AI for document summarization")
    ...
    users >> Edge(minlen="2") >> demo_app >> Edge(minlen="4") >> download_file
